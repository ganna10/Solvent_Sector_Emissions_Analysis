    ...
    foreach my $speciation (@speciations) {
        if ($mechanism eq "MCM") {
            ...
            foreach my $dir (@dirs) {
                ...
                my $kpp = KPP->new($eqn);
                $data{$mechanism}{$speciation}{$dir} = get_data($mecca, $kpp, $mechanism, $speciation, $no_runs);
            }
        } else {
...
foreach my $speciation (sort keys %{$data{"MCM"}}) {
    my %allocated;
    foreach my $dir (sort keys %{$data{'MCM'}{$speciation}}) {
...
foreach my $mechanism (sort keys %data) {
    ...
    foreach my $speciation (keys %{$data{$mechanism}}) {
        ...
        foreach my $VOC (keys %{$data{$mechanism}{$speciation}}) {
            #print "$VOC\n";
            #print "\t\t\t$VOC : $data{$mechanism}{$speciation}{$VOC}\n";
            $R->set('voc', $VOC);
            $R->set('reactivity', $data{$mechanism}{$speciation}{$VOC});
            $R->run(q` pre[voc] = reactivity `);
        }
        $R->run(q` pre = gather(pre, VOC, Reactivity, -Speciation, -Mechanism) `,
...
$R->run(q` data$VOC = factor(data$VOC, levels = c("Acids", "Alcohols", "Aldehydes", "Benzene", "Butanes", "CO", "Chlorinated", "Esters", "Ethane", "Ethene", "Ethers", "Higher alkanes", "Inorganic", "Ketones", "Methane", "Other alkenes, alkynes, dienes", "Other aromatics", "Pentanes", "Propane", "Propene", "Terpenes", "Toluene", "Trimethylbenzenes", "Xylenes")) `);
...
$R->run(q` plot = ggplot(data, aes(x = Speciation, y = Reactivity, fill = VOC)) `,
...
$R->run(q` CairoPDF(file = "Total_OH_reactivity.pdf", width = 8.7, height = 6) `,
        q` print(plot) `,
        q` dev.off() `,
);
...
sub get_data {
    my ($mecca, $kpp, $mechanism, $speciation, $no_runs) = @_;
