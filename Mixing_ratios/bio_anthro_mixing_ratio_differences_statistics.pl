...
my $mecca = MECCA->new("$base/RADM2/DE94_bio_and_anthro/boxmodel");
...
$R->run(q` write.table(entire.data, file = "bio_and_anthro_Ozone_time_series_entire_difference_statistics_by_mechanism.txt", row.names = FALSE, quote = FALSE, sep = "\t") `);
...
$R->run(q` write.table(entire.data, file = "bio_and_anthro_Ozone_time_series_entire_difference_statistics_by_speciation.txt", row.names = FALSE, quote = FALSE, sep = "\t") `);
