...
my @runs = qw( mean_NO_source_Solvents_Only );
my %data;

my $mecca = MECCA->new("$base/RADM2/DE94_mean_NO_source_Solvents_Only/boxmodel");
...
$R->run(q` write.table(d, file = "mean_NO_source_ozone_mixing_ratios.csv", quote = FALSE, row.names = FALSE, sep = ",") `);
...
#);
#$R->run(q` write.table(entire.data, file = "mean_NO_source_Solvents_Only_Ozone_time_series_entire_difference_statistics_by_mechanism.txt", row.names = FALSE, quote = FALSE, sep = "\t") `);
#
...
#);
#$R->run(q` write.table(print.data, file = "mean_NO_source_Solvents_Only_Ozone_time_series_daily_difference_statistics_by_mechanism.txt", row.names = FALSE, quote = FALSE, sep = "\t") `);
#
...
#        q` p = p + theme_hc() `,
#        q` p = p + ggtitle("mean NO source Solvents Only - by Mechanism") `,
...
#);
#
#$R->run(q` CairoPDF(file = "mean_NO_source_Solvents_only_Ozone_statistics_plots_by_mechanism.pdf", width = 11, height = 7) `,
#        q` print(p) `,
#        q` dev.off() `,
#);
#
...
#        q` entire.data$Average = sprintf("%.3e", entire.data$Average) `,
#);
#$R->run(q` write.table(entire.data, file = "mean_NO_source_Solvents_Only_Ozone_time_series_entire_difference_statistics_by_speciation.txt", row.names = FALSE, quote = FALSE, sep = "\t") `);
#
...
#        q` print.data$Average = sprintf("%.3e", print.data$Average) `,
#);
#$R->run(q` write.table(print.data, file = "mean_NO_source_Solvents_Only_Ozone_time_series_daily_difference_statistics_by_speciation.txt", row.names = FALSE, quote = FALSE, sep = "\t") `);
#
...
#        q` p = p + facet_wrap( ~ Variable, scales = "free_y") `,
#        q` p = p + theme_hc() `,
#        q` p = p + ggtitle("mean NO source Solvents Only - by Speciation") `,
#        q` p = p + theme(axis.line = element_line(colour = "black")) `,
#        q` p = p + theme(strip.text = element_text(face = "bold")) `,
#        q` p = p + theme(strip.background = element_blank()) `,
#);
#
#$R->run(q` CairoPDF(file = "mean_NO_source_Solvents_only_Ozone_statistics_plots_by_speciation.pdf", width = 11, height = 7) `,
#        q` print(p) `,
#        q` dev.off() `,
#);
#
