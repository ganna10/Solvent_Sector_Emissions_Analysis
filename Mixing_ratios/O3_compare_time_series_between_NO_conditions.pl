#! /usr/bin/env perl
# Compare O3 mixing ratios in each speciation and mechanism between tuned and mean NO runs
...
$R->run(q` plot = ggplot(data, aes(x = Time, y = Mixing.Ratio, colour = run, group = run)) `,
...
$R->run(q` CairoPDF(file = "O3_mixing_ratio_time_series_comparison_between_NO_sources.pdf", width = 11, height = 5) `,
        q` print(plot) `,
        q` dev.off() `,
);
