#! /usr/bin/env perl
# allocate cumulative Ox production from tagged MOZART runs to VOC functional groups
...
foreach my $run (@tagged_runs) {
    ...
    $plot_data{$label}{$_} = $production_rates{"Ox"}{$_}->sumover foreach (keys %{$production_rates{"Ox"}}) ;
}
...
$R->run(q` plot = ggplot(data = data, aes(x = Group, y = Ratio)) `,
        ...
        #q` plot = plot + scale_y_continuous(label = percent) `,
