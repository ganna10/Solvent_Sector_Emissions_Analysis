    ...
    foreach my $species (qw( Ox HO2x )) { 
        ...
        for (0..$#$producers) { #get rates for all producing reactions
            ...
            if (defined $parent) { # for tagged reactions
                if ($parent ~~ @acids) {
                    $string = $parent;
                } elsif ($parent ~~ @alkanes or $parent ~~ @aromatics or $parent ~~ @alkenes or $parent ~~ @alcohols or $parent ~~ @carbonyls or $parent ~~ @alkynes) {
                    next;
                } else {
...
$R->run(q` my.colours = c( "HCOOH" = "#6c254f", "CH3COOH" = "#f9c500") `,
        ...
        q` data$Acid = factor(data$Acid, levels = rev(c("HCOOH", "CH3COOH"))) `,
);
