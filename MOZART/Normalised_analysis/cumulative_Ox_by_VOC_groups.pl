#! /usr/bin/env perl
# allocate cumulative Ox production from tagged MOZART runs to VOC functional groups. Groups of emitted VOCs and normalised by their emissions
...
#closedir DIR;
my @tagged_runs = qw( EMEP_tagged_solvents_only );
...
foreach my $run (@tagged_runs) {
    ...
    foreach (keys %{$production_rates{"Ox"}}) {
        next if ($_ =~ /\+/);
        my $cumulative = $production_rates{"Ox"}{$_}->sumover;
        $cumulative /= $emissions{$_};
        $plot_data{$label}{$_} = $cumulative;
    }
}
...
$R->run(q` plot = ggplot(data = data, aes(x = Group, y = Ratio)) `,
        ...
        q` plot = plot + scale_x_discrete(limits = rev(c("Alkanes", "Alkenes", "Aromatics", "Carbonyls", "Alcohols", "Acids", "Alkynes"))) `,
        q` plot = plot + ylab("\nPercentage of Total Normalised Cumulative Ox Production\n") `,
        q` plot = plot + xlab("\n") `,
...
$R->run(q` CairoPDF(file = "MOZART_cumulative_Ox_budget_by_group.pdf", width = 200, height = 141) `,
        q` print(plot) `,
        q` dev.off() `,
);
