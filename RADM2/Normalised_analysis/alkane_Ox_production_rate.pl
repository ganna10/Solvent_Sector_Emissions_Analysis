#! /usr/bin/env perl
# allocate alkane Ox production from tagged RADM2 runs to VOC 
...
foreach my $speciation (keys %plot_data) {
    ...
    foreach my $alkane (sort keys %{$plot_data{$speciation}}) {
        next if ($alkane =~ /\+/ or $alkane eq "CH4");
        $R->set('alkane', $alkane);
        $R->set('rate', [map { $_ } $plot_data{$speciation}{$alkane}->dog]);
...
$R->run(q` my.colours = c( "ETH" = "#6c254f", "HC3" = "#f9c500", "HC5" = "#0e5628", "HC8" = "#ef6638") `,
        ...
        q` data$Alkane = factor(data$Alkane, levels = c("ETH", "HC3", "HC5", "HC8")) `,
...
$R->run(q` CairoPDF(file = "RADM2_alkanes_Ox_budget.pdf", width = 200, height = 141) `,
        q` print(plot) `,
        q` dev.off() `,
);
