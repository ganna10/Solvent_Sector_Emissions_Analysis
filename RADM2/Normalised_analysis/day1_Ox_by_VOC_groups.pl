#! /usr/bin/env perl
# allocate day1 Ox production from tagged RADM2 runs to VOC functional groups. Groups of emitted VOCs and normalised by their emissions
...
$R->run(q` CairoPDF(file = "RADM2_day1_Ox_budget_by_group.pdf", width = 200, height = 141) `,
        q` print(plot) `,
        q` dev.off() `,
);
