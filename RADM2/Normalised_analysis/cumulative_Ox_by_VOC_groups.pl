#! /usr/bin/env perl
# allocate cumulative Ox production from tagged RADM2 runs to VOC functional groups. Groups of emitted VOCs and normalised by their emissions
...
$R->run(q` plot = ggplot(data = data, aes(x = Group, y = Ratio)) `,
        ...
        q` plot = plot + scale_x_discrete(limits = rev(c("Alkanes", "Alkenes", "Aromatics", "Carbonyls"))) `,
...
$R->run(q` CairoPDF(file = "RADM2_cumulative_Ox_budget_by_group.pdf", width = 200, height = 141) `,
        q` print(plot) `,
        q` dev.off() `,
);
