#! /usr/bin/env perl
# allocate alkene Ox production from tagged RADM2 runs to VOC 
    ...
    foreach my $species (qw( Ox HO2x )) { 
        ...
        for (0..$#$producers) { #get rates for all producing reactions
            ...
            if (defined $parent) { # for tagged reactions
                if ($parent ~~ @alkenes) {
                    $string = $parent;
                } elsif ($parent eq "CH4" or $parent ~~ @alkanes or $parent ~~ @aromatics or $parent ~~ @carbonyls ) {
                    next;
                } else {
...
foreach my $speciation (keys %plot_data) {
    ...
    foreach my $alkene (sort keys %{$plot_data{$speciation}}) {
        next if ($alkene =~ /\+/);
        $R->set('alkene', $alkene);
        $R->set('rate', [map { $_ } $plot_data{$speciation}{$alkene}->dog]);
...
$R->run(q` my.colours = c( "OL2" = "#6c254f", "OLI" = "#f9c500", "OLT" = "#0e5628", "ISO" = "#ef6638") `,
        q` data$Alkene = factor(data$Alkene, levels = c("OL2", "OLI", "OLT", "ISO")) `,
...
$R->run(q` CairoPDF(file = "RADM2_alkenes_Ox_budget.pdf", width = 200, height = 141) `,
        q` print(plot) `,
        q` dev.off() `,
);
