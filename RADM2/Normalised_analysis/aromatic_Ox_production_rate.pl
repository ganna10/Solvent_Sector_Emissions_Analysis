#! /usr/bin/env perl
# allocate aromatic Ox production from tagged RADM2 runs to VOC 
    ...
    foreach my $species (qw( Ox HO2x )) { 
        ...
        for (0..$#$producers) { #get rates for all producing reactions
            ...
            if (defined $parent) { # for tagged reactions
                if ($parent ~~ @aromatics) {
                    $string = $parent;
                } elsif ($parent eq "CH4" or $parent ~~ @alkanes or $parent ~~ @alkenes or $parent ~~ @carbonyls ) {
                    next;
                } else {
...
foreach my $speciation (keys %plot_data) {
    ...
    foreach my $aromatic (sort keys %{$plot_data{$speciation}}) {
        next if ($aromatic =~ /\+/);
        $R->set('aromatic', $aromatic);
        $R->set('rate', [map { $_ } $plot_data{$speciation}{$aromatic}->dog]);
...
$R->run(q` my.colours = c( "TOL" = "#6c254f", "XYL" = "#f9c500") `,
        q` data$Aromatic = factor(data$Aromatic, levels = c("TOL", "XYL")) `,
...
$R->run(q` CairoPDF(file = "RADM2_aromatics_Ox_budget.pdf", width = 200, height = 141) `,
        q` print(plot) `,
        q` dev.off() `,
);
