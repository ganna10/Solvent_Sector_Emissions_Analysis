...
foreach my $mechanism (@mechanisms) {
    foreach my $speciation (@speciations) {
        foreach my $run (@runs) {
            ...
            $data{$mechanism}{$run}{$speciation} = $mr(1:$ntime-2);
        }
    }
}
...
foreach my $mechanism (sort keys %data) {
    foreach my $run (sort keys %{$data{$mechanism}}) {
        ...
        foreach my $speciation (sort keys %{$data{$mechanism}{$run}}) {
            $R->set('speciation', $speciation);
            $R->set('mixing.ratio', [ map { $_ } $data{$mechanism}{$run}{$speciation}->dog ]);
            $R->run(q` data[speciation] = mixing.ratio `);
        }
        $R->set('filename', "${mechanism}_${run}_${species}_mixing_ratios.pdf");
        $R->set('title', "Mixing Ratios: using $mechanism and $run");
        $R->run(q` data = gather(data, Speciation, Mixing.Ratio, -Time) `);
        ...
        $R->run(q` Cairo(file = filename, type = "pdf", bg = "transparent", units = "in", width = 8.6, height = 6) `,
                q` print(plot) `,
                q` dev.off() `,
        );
    }
