#! /usr/bin/env perl
# Correlate fraction of alkanes, aromatics and oxygenated of VOC emissions to first day Ox production in each mechanism
...
$R->run(q` plot = ggplot(alkanes, aes(x = Emission.Fraction, y = Ox, colour = Speciation)) `,
        ...
        q` plot = plot + geom_text(data = alkanes.eqn, aes(x = 0.45, y = 2e9, label = Label), colour = "black", inherit.aes = FALSE, parse = TRUE) + geom_smooth(method = "lm", se = FALSE, colour = "black") `,
        #q` plot = list(plot) `,
);
...
$R->run(q` plot.1 = ggplot(aromatics, aes(x = Emission.Fraction, y = Ox, colour = Speciation)) `,
        ...
        q` plot.1 = plot.1 + scale_y_continuous(limits = c(1e9, 2e9)) `,
        ...
        q` plot.1 = plot.1 + geom_text(data = aromatics.eqn, aes(x = 0.45, y = 2e9, label = Label), colour = "black", inherit.aes = FALSE, parse = TRUE) + geom_smooth(method = "lm", se = FALSE, colour = "black") `,
...
$R->run(q` plot.2 = ggplot(oxygenated, aes(x = Emission.Fraction, y = Ox, colour = Speciation)) `,
        ...
        q` plot.2 = plot.2 + scale_y_continuous(limits = c(1e9, 2e9)) `,
        ...
        q` plot.2 = plot.2 + geom_text(data = oxygenated.eqn, aes(x = 0.45, y = 2e9, label = Label), colour = "black", inherit.aes = FALSE, parse = TRUE) + geom_smooth(method = "lm", se = FALSE, colour = "black") `,
        #q` plot.2 = list(plot.2) `,
);

$R->run(q` plots = c(list(plot), list(plot.1), list(plot.2)) `);
$R->run(q` CairoPDF(file = "Ox_production_vs_alkane_aromatic_oxygenated_VOC_emission_fractions_first_day.pdf", width = 10, height = 15) `);
