...
alcohol.data = filter(mcm.Ox.production, Types == "Alcohols")
mcm.speciations %>% filter(Types == "Alcohols")
alcohol.data
alcohol.data$Contribution = c(32, 22.517225, 27.927928, 10.4, 3.196803, 9.198637, 22.745301, 16.405235)

p = ggplot(alcohol.data, aes(x = Contribution, y = Ox.Production, colour = Speciation))
p = p + geom_point(size = 4)
p + geom_text(aes(x = 14, y = 1.3e9, label = lm_eqn(lm(Ox.Production ~ Contribution, alcohol.data))), colour = "black", parse = TRUE) + geom_smooth(method = "lm", se = FALSE, colour = "black")

alcohol.data$Contribution = alcohol.data$Contribution/100
alcohol.data$Contribution = alcohol.data$Contribution * 5.74e-12
