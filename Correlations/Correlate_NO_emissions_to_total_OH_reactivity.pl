#! /usr/bin/env perl
# Correlate cumulative NO emissions to total OH reactivity
...
my (%NO_emissions, %total_reactivity);
...
$R->run(q` plot = ggplot(data, aes(x = NO.Emissions, y = Total.Reactivity, colour = Speciation)) `,
...
$R->run(q` CairoPDF(file = "NO_emissions_vs_total_reactivity_facet_mechanism.pdf", width = 8.7, height = 6) `,
        q` print(plot) `,
        q` dev.off() `,
);

$R->run(q` plot = ggplot(data, aes(x = NO.Emissions, y = Total.Reactivity, colour = Mechanism)) `,
        q` plot = plot + geom_point(size = 4) `,
...
$R->run(q` CairoPDF(file = "NO_emissions_vs_total_reactivity_facet_speciation.pdf", width = 8.7, height = 6) `,
        q` print(plot) `,
        q` dev.off() `,
);
