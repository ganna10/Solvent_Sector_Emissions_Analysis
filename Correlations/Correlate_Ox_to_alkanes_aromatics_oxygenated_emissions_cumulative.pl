#! /usr/bin/env perl
# Correlate fraction of alkanes, aromatics and oxygenated of VOC emissions to total Ox production in each mechanism
...
$R->run(q` data = data %>% filter(Type == "Alkanes" | Type == "Aromatics" | Type == "Oxygenated") `);
...
$R->run(q` alkanes = data %>% filter(Type == "Alkanes") %>% select(-Type) `);
$R->run(q` alkanes.eqn = alkanes %>% group_by(Mechanism) %>% do(mod2 = cor(.$Ox, .$Emission.Fraction, method = "pearson")) %>% mutate(Label = paste('"r = ', sprintf('%1.3f', mod2[1]), '"')) %>% select(-mod2) `);

$R->run(q` plot = ggplot(alkanes, aes(x = Emission.Fraction, y = Ox, colour = Speciation)) `,
        q` plot = plot + geom_point(size = 4) `,
        q` plot = plot + facet_wrap( ~ Mechanism, scales = "free") `,
        q` plot = plot + plot.lines() `,
        q` plot = plot + scale_x_continuous(label = percent, limits = c(0, 0.75)) `,
        q` plot = plot + scale_y_continuous(limits = c(7e9, 9e9)) `,
        ...
        q` plot = plot + geom_text(data = alkanes.eqn, aes(x = 0.45, y = 8.7e9, label = Label), colour = "black", inherit.aes = FALSE, parse = TRUE) + geom_smooth(method = "lm", se = FALSE, colour = "black") `,
        #q` plot = list(plot) `,
);

$R->run(q` aromatics = data %>% filter(Type == "Aromatics") %>% select(-Type) `);
$R->run(q` aromatics.eqn = aromatics %>% group_by(Mechanism) %>% do(mod2 = cor(.$Ox, .$Emission.Fraction, method = "pearson")) %>% mutate(Label = paste('"r = ', sprintf('%1.3f', mod2[1]), '"')) %>% select(-mod2) `);

$R->run(q` plot.1 = ggplot(aromatics, aes(x = Emission.Fraction, y = Ox, colour = Speciation)) `,
        q` plot.1 = plot.1 + geom_point(size = 4) `,
        q` plot.1 = plot.1 + facet_wrap( ~ Mechanism, scales = "free") `,
        q` plot.1 = plot.1 + plot.lines() `,
        q` plot.1 = plot.1 + scale_x_continuous(label = percent, limits = c(0, 0.75)) `,
        q` plot.1 = plot.1 + scale_y_continuous(limits = c(7e9, 9e9)) `,
        q` plot.1 = plot.1 + xlab("Percent Emissions") `,
        q` plot.1 = plot.1 + scale_colour_manual(values = speciation.colours, limits = levels(data$Speciation)) `,
        q` plot.1 = plot.1 + theme(legend.position = "top") `,
        q` plot.1 = plot.1 + geom_text(data = aromatics.eqn, aes(x = 0.45, y = 8.7e9, label = Label), colour = "black", inherit.aes = FALSE, parse = TRUE) + geom_smooth(method = "lm", se = FALSE, colour = "black") `,
        #q` plot.1 = list(plot.1) `,
);

$R->run(q` oxygenated = data %>% filter(Type == "Oxygenated") %>% select(-Type) `);
$R->run(q` oxygenated.eqn = oxygenated %>% group_by(Mechanism) %>% do(mod2 = cor(.$Ox, .$Emission.Fraction, method = "pearson")) %>% mutate(Label = paste('"r = ', sprintf('%1.3f', mod2[1]), '"')) %>% select(-mod2) `);

$R->run(q` plot.2 = ggplot(oxygenated, aes(x = Emission.Fraction, y = Ox, colour = Speciation)) `,
        q` plot.2 = plot.2 + geom_point(size = 4) `,
        q` plot.2 = plot.2 + facet_wrap( ~ Mechanism, scales = "free") `,
        q` plot.2 = plot.2 + plot.lines() `,
        q` plot.2 = plot.2 + scale_x_continuous(label = percent, limits = c(0, 0.75)) `,
        q` plot.2 = plot.2 + scale_y_continuous(limits = c(7e9, 9e9)) `,
        q` plot.2 = plot.2 + xlab("Percent Emissions") `,
        q` plot.2 = plot.2 + scale_colour_manual(values = speciation.colours, limits = levels(data$Speciation)) `,
        q` plot.2 = plot.2 + theme(legend.position = "top") `,
        q` plot.2 = plot.2 + geom_text(data = oxygenated.eqn, aes(x = 0.45, y = 8.7e9, label = Label), colour = "black", inherit.aes = FALSE, parse = TRUE) + geom_smooth(method = "lm", se = FALSE, colour = "black") `,
        #q` plot.2 = list(plot.2) `,
);

$R->run(q` plots = c(list(plot), list(plot.1), list(plot.2)) `);
$R->run(q` CairoPDF(file = "Ox_production_vs_alkane_aromatic_oxygenated_VOC_emission_fractions_all_data.pdf", width = 10, height = 15) `);
$R->run(q` multiplot = grid.arrange(plots[[1]] + theme(axis.title = element_blank()), 
                                    plots[[2]] + theme(axis.title.x = element_blank(), legend.position = "none"), 
                                    plots[[3]] + theme(legend.position = "none", axis.title.y = element_blank())) `,
                                                #left = textGrob("Ox Production (molecules cm-3)"), gp = gpar(fontface = "bold"), rot = 90, vjust = 0.5) `,
);

$R->run(#q` CairoPDF(file = "Ox_production_vs_alkane_aromatic_oxygenated_VOC_emission_fractions_all_data.pdf", width = 10, height = 21) `,
        q` print(multiplot) `,
        q` dev.off() `,
);
