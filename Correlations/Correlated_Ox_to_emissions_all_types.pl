#! /usr/bin/env perl
# Correlate fraction of all types of VOC emissions to total Ox production in all mechanisms
...
$R->run(q` CairoPDF(file = "Ox_production_vs_all_VOC_emission_fractions_all_data.pdf", width = 7, height = 10) `,
        q` print(plot) `,
        q` dev.off() `,
);
