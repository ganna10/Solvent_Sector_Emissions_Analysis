...
foreach my $mechanism (@mechanisms) {
    foreach my $speciation (@speciations) {
        foreach my $run (@runs) {
            ...
            $O3 = $O3(72:144); #get 2nd mixing ratios
...
foreach my $mechanism (sort keys %data) {
    foreach my $run (sort keys %{$data{$mechanism}}) {
        ...
        $min_max{$mechanism}{$print_run}{"Maximum"}{$sorted[0]} = $data{$mechanism}{$run}{$sorted[0]}; 
        $min_max{$mechanism}{$print_run}{"Minimum"}{$sorted[-1]} = $data{$mechanism}{$run}{$sorted[-1]}; 
        $min_max{$mechanism}{$print_run}{"Difference"}{"difference"} = sprintf "%.2f", $data{$mechanism}{$run}{$sorted[0]} - $data{$mechanism}{$run}{$sorted[-1]};
    }
}
...
foreach my $mechanism (sort keys %min_max) {
    foreach my $run (sort keys %{$min_max{$mechanism}}) {
        print $out "\t\t\t$mechanism & $run & $min_max{$mechanism}{$run}{'Difference'}{'difference'} & ", keys %{$min_max{$mechanism}{$run}{"Maximum"}}, " & ", keys %{$min_max{$mechanism}{$run}{"Minimum"}}, "\\\\ \n";
    }
