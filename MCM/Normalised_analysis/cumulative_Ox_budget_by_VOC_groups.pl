#! /usr/bin/env perl
# allocate cumulative Ox production from tagged MCM runs to parent VOCs functional groups. Normalised by VOC emissions, no Inorganic or CH4 included
...
foreach my $run (sort keys %production_rates) {
    foreach (sort keys %{$production_rates{$run}{"Ox"}}) {
        ...
        my $cumulative = $production_rates{$run}{"Ox"}{$_}->sumover;
        $cumulative /= $emissions{$run}{$_};
        $plot_data{$run}{$_} = $cumulative;
    }
}
