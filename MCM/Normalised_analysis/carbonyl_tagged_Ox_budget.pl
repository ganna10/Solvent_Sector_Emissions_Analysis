#! /usr/bin/env perl
# allocate carbonyls Ox production from tagged MCM runs. Normalised by VOC emissions, no Inorganic or CH4 included 
    ...
    foreach my $species (qw( Ox HO2x )) { 
        ...
        for (0..$#$producers) { #get rates for all producing reactions
            ...
            if (defined $parent) { # for tagged reactions
                if ($parent ~~ @carbonyls) {
                    $string = $parent;
                } elsif ($parent eq "CH4" or $parent ~~ @alkanes or $parent ~~ @alkenes or $parent ~~ @alcohols or $parent ~~ @aromatics or $parent ~~ @acids or $parent ~~ @esters or $parent ~~ @ethers or $parent ~~ @alkynes or $parent ~~ @chlorinated ) {
                    next;
                } else {
...
foreach my $run (keys %plot_data) {
    ...
    foreach my $carbonyl (sort keys %{$plot_data{$run}}) {
        $R->set('carbonyl', $carbonyl);
        $R->set('rate', [map { $_ } $plot_data{$run}{$carbonyl}->dog]);
