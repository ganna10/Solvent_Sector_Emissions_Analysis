#! /usr/bin/env perl
# allocate alkenes Ox production from tagged MCM runs. Normalised by VOC emissions, no Inorganic or CH4 included 
    ...
    foreach my $species (qw( Ox HO2x )) { 
        ...
        for (0..$#$producers) { #get rates for all producing reactions
            ...
            if (defined $parent) { # for tagged reactions
                if ($parent ~~ @alkenes) {
                    $string = $parent;
                } elsif ($parent eq "CH4" or $parent ~~ @alkanes or $parent ~~ @aromatics or $parent ~~ @alcohols or $parent ~~ @carbonyls or $parent ~~ @acids or $parent ~~ @esters or $parent ~~ @ethers or $parent ~~ @alkynes or $parent ~~ @chlorinated ) {
                    next;
                } else {
