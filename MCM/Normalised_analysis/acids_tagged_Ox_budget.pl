#! /usr/bin/env perl
# allocate acids Ox production from tagged MCM runs. Normalised by VOC emissions, no Inorganic or CH4 included 
    ...
    foreach my $species (qw( Ox HO2x )) { 
        ...
        for (0..$#$producers) { #get rates for all producing reactions
            ...
            if (defined $parent) { # for tagged reactions
                if ($parent ~~ @acids) {
                    $string = $parent;
                } elsif ($parent eq "CH4" or $parent ~~ @alkanes or $parent ~~ @alkenes or $parent ~~ @carbonyls or $parent ~~ @aromatics or $parent ~~ @alcohols or $parent ~~ @esters or $parent ~~ @ethers or $parent ~~ @alkynes or $parent ~~ @chlorinated ) {
                    next;
                } else {
...
foreach my $run (keys %plot_data) {
    ...
    foreach my $acid (sort keys %{$plot_data{$run}}) {
        $R->set('acid', $acid);
        $R->set('rate', [map { $_ } $plot_data{$run}{$acid}->dog]);
        $R->run(q` pre[acid] = rate `);
    }
    $R->set('speciation', $run);
    $R->run(q` pre$Speciation = rep(speciation, length(Time)) `,
            q` pre = melt(pre, id.vars = c("Time", "Speciation"), variable.name = "Acid", value.name = "Rate") `,
...
$R->run(q` scientific_10 <- function(x) { parse(text=gsub("e", " %*% 10^", scientific_format()(x))) } `, #scientific label format for y-axis
        q` my.colours = c( "HCOOH" = "#6c254f", "CH3CO2H" = "#0e5628", "PROPACID" = "#ef6638") `,
        q` data$Acid = factor(data$Acid, levels = c("HCOOH", "CH3CO2H", "PROPACID" )) `,
        q` data = ddply(data, .(Acid)) `,
);

$R->run(q` plot = ggplot(data = data, aes(x = Time, y = Rate, fill = Acid)) `,
        ...
        q` plot = plot + scale_fill_manual(values = my.colours, limits = rev(levels(data$Acid))) `,
);

$R->run(q` CairoPDF(file = "MCM_Ox_budget_acids.pdf", width = 200, height = 141) `,
        q` print(plot) `,
        q` dev.off() `,
);
