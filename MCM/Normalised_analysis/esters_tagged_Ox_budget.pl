#! /usr/bin/env perl
# allocate esters Ox production from tagged MCM runs. Normalised by VOC emissions, no Inorganic or CH4 included 
    ...
    foreach my $species (qw( Ox HO2x )) { 
        ...
        for (0..$#$producers) { #get rates for all producing reactions
            ...
            if (defined $parent) { # for tagged reactions
                if ($parent ~~ @esters) {
                    $string = $parent;
                } elsif ($parent eq "CH4" or $parent ~~ @alkanes or $parent ~~ @alkenes or $parent ~~ @carbonyls or $parent ~~ @aromatics or $parent ~~ @alcohols or $parent ~~ @acids or $parent ~~ @ethers or $parent ~~ @alkynes or $parent ~~ @chlorinated ) {
                    next;
                } else {
...
foreach my $run (keys %plot_data) {
    ...
    foreach my $ester (sort keys %{$plot_data{$run}}) {
        $R->set('ester', $ester);
        $R->set('rate', [map { $_ } $plot_data{$run}{$ester}->dog]);
        $R->run(q` pre[ester] = rate `);
    }
    $R->set('speciation', $run);
    $R->run(q` pre$Speciation = rep(speciation, length(Time)) `,
            q` pre = melt(pre, id.vars = c("Time", "Speciation"), variable.name = "Ester", value.name = "Rate") `,
...
$R->run(q` scientific_10 <- function(x) { parse(text=gsub("e", " %*% 10^", scientific_format()(x))) } `, #scientific label format for y-axis
        q` my.colours = c( "ETHACET" = "#6c254f", "NPROACET" = "#f9c500", "IPROACET" = "#0e5628", "NBUTACET" = "#ef6638") `,
        q` data$Ester = factor(data$Ester, levels = c("ETHACET", "NPROACET", "IPROACET", "NBUTACET" )) `,
        q` data = ddply(data, .(Ester)) `,
);

$R->run(q` plot = ggplot(data = data, aes(x = Time, y = Rate, fill = Ester)) `,
        ...
        q` plot = plot + scale_fill_manual(values = my.colours, limits = rev(levels(data$Ester))) `,
);

$R->run(q` CairoPDF(file = "MCM_Ox_budget_esters.pdf", width = 200, height = 141) `,
        q` print(plot) `,
        q` dev.off() `,
);
