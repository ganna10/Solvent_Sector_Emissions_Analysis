#! /usr/bin/env perl
# allocate alcohols Ox production from tagged MCM runs. Normalised by VOC emissions, no Inorganic or CH4 included 
    ...
    foreach my $species (qw( Ox HO2x )) { 
        ...
        for (0..$#$producers) { #get rates for all producing reactions
            ...
            if (defined $parent) { # for tagged reactions
                if ($parent ~~ @alcohols) {
                    $string = $parent;
                } elsif ($parent eq "CH4" or $parent ~~ @alkanes or $parent ~~ @alkenes or $parent ~~ @carbonyls or $parent ~~ @aromatics or $parent ~~ @acids or $parent ~~ @esters or $parent ~~ @ethers or $parent ~~ @alkynes or $parent ~~ @chlorinated ) {
                    next;
                } else {
...
foreach my $run (keys %plot_data) {
    ...
    foreach my $alcohol (sort keys %{$plot_data{$run}}) {
        $R->set('alcohol', $alcohol);
        $R->set('rate', [map { $_ } $plot_data{$run}{$alcohol}->dog]);
        $R->run(q` pre[alcohol] = rate `);
    }
    $R->set('speciation', $run);
    $R->run(q` pre$Speciation = rep(speciation, length(Time)) `,
            q` pre = melt(pre, id.vars = c("Time", "Speciation"), variable.name = "Alcohol", value.name = "Rate") `,
