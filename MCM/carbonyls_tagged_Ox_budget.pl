#! /usr/bin/env perl
# allocate carbonyls Ox production from tagged MCM runs
    ...
    foreach my $species (qw( Ox HO2x )) { 
        ...
        for (0..$#$producers) { #get rates for all producing reactions
            ...
            if (defined $parent) { # for tagged reactions
                if ($parent ~~ @carbonyls) {
                    $string = $parent;
                } elsif ($parent ~~ @alkenes or $parent ~~ @aromatics or $parent ~~ @alcohols or $parent ~~ @alkanes or $parent ~~ @acids or $parent ~~ @esters or $parent ~~ @ethers or $parent ~~ @alkynes or $parent ~~ @chlorinated or $parent eq "CH4") {
                    next;
                } else {
