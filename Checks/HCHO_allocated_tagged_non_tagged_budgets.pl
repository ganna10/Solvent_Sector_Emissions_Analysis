...
# Version 1: Jane Coates 30/3/2015 allocating to categories
# Version 2: Jane Coates 30/3/2015 not including notag
...
sub get_data {
    ...
    foreach my $process (keys %production_rates) {
        ...
        if (exists $category_mapping{$mechanism}) {
            ...
            my $category = get_category($process, $mechanism, $speciation);
            if ($process eq "CH4") {
                $final_categories{$category} += $production_rates{$process} / $no_dirs;
            } else {
                $final_categories{$category} += $production_rates{$process};
            }
        }
