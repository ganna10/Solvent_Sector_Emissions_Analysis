...
sub get_data {
    ...
    foreach my $reaction (keys %{$production_rates{'HO2x'}}) {
        ...
        $consumption_rates{"Ox"}{$reaction} += $consumption_rates{"Ox"}{'HO2 + NO3'} * $consumption_rates{'HO2x'}{$reaction} / $total_ho2x_production; 
    }
    ...
    foreach my $process (keys %{$production_rates{"Ox"}}) {
        #print "$process\n";
        #print "$process: ", $production_rates{"Ox"}{$process}, "\n";
