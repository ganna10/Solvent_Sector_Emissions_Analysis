#! /usr/bin/env perl
# allocate alcohol Ox production from tagged MOZART runs to VOC 
    ...
    foreach my $species (qw( Ox HO2x )) { 
        ...
        for (0..$#$producers) { #get rates for all producing reactions
            ...
            if (defined $parent) { # for tagged reactions
                if ($parent ~~ @alcohols) {
                    $string = $parent;
                } elsif ($parent eq "CH4" or $parent ~~ @alkenes or $parent ~~ @aromatics or $parent ~~ @carbonyls or $parent ~~ @alkanes or $parent ~~ @acids or $parent ~~ @alkynes) {
                    next;
                } else {
