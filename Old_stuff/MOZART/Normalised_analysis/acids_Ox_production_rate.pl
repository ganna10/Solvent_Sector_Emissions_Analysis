    ...
    foreach my $species (qw( Ox HO2x )) { 
        ...
        for (0..$#$producers) { #get rates for all producing reactions
            ...
            if (defined $parent) { # for tagged reactions
                if ($parent ~~ @acids) {
                    $string = $parent;
                } elsif ($parent eq "CH4" or $parent ~~ @alkanes or $parent ~~ @aromatics or $parent ~~ @carbonyls or $parent ~~ @alcohols or $parent ~~ @alkenes or $parent ~~ @alkynes) {
                    next;
                } else {
...
$R->run(q` CairoPDF(file = "MOZART_acids_Ox_budget.pdf", width = 200, height = 141) `,
        q` print(plot) `,
        q` dev.off() `,
);
