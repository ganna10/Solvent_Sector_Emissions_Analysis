    ...
    foreach my $species (qw( Ox HO2x )) { 
        ...
        for (0..$#$producers) { #get rates for all producing reactions
            ...
            if (defined $parent) { # for tagged reactions
                if ($parent ~~ @carbonyls) {
                    $string = $parent;
                } elsif ($parent ~~ @alkanes or $parent ~~ @alkenes or $parent ~~ @aromatics) {
                    next;
                } else {
...
$R->run(q` my.colours = c( "KET" = "#6c254f", "ALD" = "#0e5628", "HCHO" = "#f9c500") `,
        q` my.names = c("HCHO", "ALD", "KET") `,
