#! /usr/bin/env perl
# allocate cumulative Ox production from tagged MCM runs to parent VOCs functional groups
...
foreach my $run (sort keys %production_rates) {
    $plot_data{$run}{$_} = $production_rates{$run}{"Ox"}{$_}->sumover foreach (keys %{$production_rates{$run}{"Ox"}});
}

foreach my $run (keys %plot_data) {
    foreach my $gp (keys %{$plot_data{$run}}) {
        my $total = 0;
        $total += $plot_data{$run}{$_} foreach (keys %{$plot_data{$run}}) ; 
        $plot_data{$run}{$_} /= $total foreach (keys %{$plot_data{$run}}) ; 
    }
}
...
$R->run(q` CairoPDF(file = "MCM_cumulative_Ox_budget_by_group.pdf", width = 200, height = 141) `,
        q` print(plot) `,
        q` dev.off() `,
);
