...
$R->run(q` plot = ggplot(data = data, aes(x = Group, y = Ratio)) `,
        ...
        q` plot = plot + scale_x_discrete(limits = rev(c("CH4", "Inorganic", "Alkanes", "Alkenes", "Aromatics", "Carbonyls", "Alcohols", "Acids", "Alkynes", "Esters", "Ethers", "Chlorinated", "Inorganic"))) `,
