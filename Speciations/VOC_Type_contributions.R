...
p = ggplot(data %>% arrange(Types), aes(x = Speciation, y = Percentage, fill = Types))
...
p = p + scale_fill_manual(values = my.colours, limits = rev(levels(data$Types)), guide = guide_legend(ncol = 1))
p

CairoPDF(file = "MCM_percent_all_VOC_b_w.pdf")
print(p)
dev.off()
