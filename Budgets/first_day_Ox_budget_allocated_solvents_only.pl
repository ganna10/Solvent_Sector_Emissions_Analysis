#! /usr/bin/env perl
# Allocate first day Ox production budgets back to categories used to defined speciations
...
$R->run(q` plot.lines = function () { list( theme_tufte() ,
                                            ggtitle("First day Ox Production Budget") ,
...
$R->run(q` CairoPDF(file = "First_day_Ox_budget_allocated_facet_mechanism.pdf", width = 8.6, height = 6) `,
        q` print(plot) `,
        q` dev.off() `,
);
...
$R->run(q` CairoPDF(file = "First_day_Ox_budget_allocated_facet_speciation.pdf", width = 8.6, height = 6) `,
        q` print(plot) `,
        q` dev.off() `,
);
...
sub get_data {
    ...
    foreach my $process (keys %{$production_rates{"Ox"}}) {
        my $reshape = $production_rates{"Ox"}{$process}->reshape($n_per_day, $n_days);
        my $integrate = $reshape->sumover;
        $production_rates{"Ox"}{$process} = $integrate;
    }
            ...
            if (exists $category_mapping{$mechanism}{$speciation}) {
                if (exists $category_mapping{$mechanism}{$speciation}{$process}) {
                    foreach my $entry (@{$category_mapping{$mechanism}{$speciation}{$process}}) {
                        ...
                        $category =~ s/_/ /g;
                        $final_categories{$category} += $factor * $production_rates{"Ox"}{$process}->at(0);
                    }
                } else {
                    my $category = get_category($process, $mechanism, $speciation);
                    $final_categories{$category} += $production_rates{'Ox'}{$process}->at(0);
                }
            }
        } else {
            my $category = get_category($process, $mechanism, $speciation);
