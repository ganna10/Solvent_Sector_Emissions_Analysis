#! /usr/bin/env perl
# Daily Esters HO2x production budgets
...
$R->run(q` CairoPDF(file = "Esters_HO2x_production_budgets_facet_speciation.pdf", width = 10, height = 7) `,
        q` print(plot) `,
        q` dev.off() `,
);
...
sub get_data {
    ...
    my %production_rates;
    my $species = "HO2x";
    my @esters = qw( ETHACET NBUTACET IPROACET NPROACET BIGALK HC3 HC5 );
    ...
    for (0..$#$producers) {
        ...
        next unless (defined $parent and $parent ~~ @esters);
    ...
    if ($mechanism eq "RADM2" or $mechanism eq "MOZART") { #allocation of RADM2 and MOZART emitted species to Higher Alkanes
        open my $in, '<:encoding(utf-8)', "Esters_RADM2_MOZ_fractional_contributions.csv" or die $!;
        my @lines = <$in>;
        close $in;
