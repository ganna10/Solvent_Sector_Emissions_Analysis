#! /usr/bin/env perl
# Plot cumulative CH3O2 budgets from CH4 degradation
...
foreach my $mechanism (sort keys %data) {
    ...
    foreach my $speciation (sort keys %{$data{$mechanism}}) {
        ...
        foreach my $reactant (sort keys %{$data{$mechanism}{$speciation}}) {
            ...
            $R->set('ch3o2x', $data{$mechanism}{$speciation}{$reactant});
            $R->run(q` pre[reactants] = ch3o2x `);
        }
        $R->run(q` pre = gather(pre, Reactants, CH3O2x, -Mechanism, -Speciation) `,
...
$R->run(q` plot = ggplot(data, aes(x = Mechanism, y = CH3O2x, fill = Reactants)) `,
...
$R->run(q` CairoPDF(file = "CH4_CH3O2x_production_reactions_facet_speciation.pdf", width = 10, height = 7) `,
        q` print(plot) `,
        q` dev.off() `,
);
