#! /usr/bin/env perl
#Compare OH consumption budgets, both total and only those from organic sources
...
$R->run(q` data$Item = factor(data$Item, levels = c("Acids", "Alcohols", "Aldehydes", "Benzene", "Butanes", "CO", "Chlorinated", "Esters", "Ethane", "Ethene", "Ethers", "Higher alkanes", "Inorganic", "Ketones", "Methane", "Other alkenes, alkynes, dienes", "Other aromatics", "Pentanes", "Propane", "Propene", "Terpenes", "Toluene", "Trimethylbenzenes", "Xylenes")) `);
...
$R->run(q` plot = ggplot(data, aes(x = Mechanism, y = OH, fill = Item)) `,
        ...
        q` plot = plot + scale_fill_manual(values = my.colours, limits = rev(levels(data$Item))) `,
);

$R->run(q` CairoPDF(file = "Total_OH_consumption_all_items_facet_speciation.pdf", width = 10, height = 7) `,
        q` print(plot) `,
        q` dev.off() `,
);
...
#);
#
#$R->run(q` CairoPDF(file = "Total_OH_consumption_organic_facet_speciation.pdf", width = 10, height = 7) `,
#        q` print(plot) `,
#        q` dev.off() `,
#);
...
sub get_data {
    ...
    for (0..$#$consumers) {
        ...
        if (defined $parent and $parent eq "notag" ) {
            $consumption_rate{"notag"} += $rate->sum;
        } elsif (defined $parent) {
            $consumption_rate{$parent} += $rate->sum;
        } else {
            my $reaction_string = $kpp->reaction_string($reaction);
            if ($reaction_string =~ /notag/) {
                $consumption_rate{"notag"} += $rate->sum;
            } else {
                $consumption_rate{$reaction_string} += $rate->sum;
            }
        }
    }
    delete $consumption_rate{"notag"};
    my %final_categories;
    foreach my $process (keys %consumption_rate) {
            ...
            if (exists $category_mapping{$mechanism}{$speciation}) {
                if (exists $category_mapping{$mechanism}{$speciation}{$process}) {
                    foreach my $entry (@{$category_mapping{$mechanism}{$speciation}{$process}}) {
                        ...
                        $category =~ s/_/ /g;
                        $final_categories{$category} += $factor * $consumption_rate{$process};
                    }
                } else {
                    my $category = get_category($process, $mechanism, $speciation);
                    $final_categories{$category} += $consumption_rate{$process};
                }
            }
        } else {
            my $category = get_category($process, $mechanism, $speciation);
    ...
    if ($mechanism eq "MCM") {
        foreach my $category (keys %final_categories) {
            $final_categories{$category} /= $no_dirs if ($category eq "Methane" or $category eq "Inorganic" or $category eq "CO");
        }
    }
