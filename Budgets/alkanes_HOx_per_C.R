...
higher.alkanes = higher.alkanes %>% group_by(Mechanism, Speciation, C.number) %>% summarise(Total.HOx = sum(HOx))
...
c4$C.number = rep(5, length(c5$HOx.Prod))
c5$C.number = rep(5, length(c5$HOx.Prod))
alks = rbind(c2, c3, c4, c5)
alks = alks %>% group_by(Mechanism, Speciation, C.number) %>% summarise(Total.HOx = sum(HOx.Prod))
alks = alks %>% mutate(HOx.per.C = Total.HOx / C.number)
alkanes = rbind(alks, higher.alkanes)
alkanes = sort(alkanes, C.number)
