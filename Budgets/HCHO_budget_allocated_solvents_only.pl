...
foreach my $mechanism (@mechanisms) {
    foreach my $speciation (@speciations) {
        if ($mechanism =~ /MOZART|RADM/) {
            ...
            my $kpp = KPP->new($eqn);
            my $spc = "$base/$mechanism/${speciation}_tagged_solvents_only/gas.spc";
            ...
            my $tagged_species = get_tagged_species($spc, $lookup);
            my $run = $mechanism;
            $data{$mechanism}{$speciation} = get_data($mecca, $kpp, $tagged_species, $mechanism, $speciation, $run);
        } else {
            ...
            foreach my $run (@runs) {
                ...
                my $kpp = KPP->new($eqn);
                my $spc = "$base/$mechanism/$run/gas.spc";
                my $tagged_species = get_tagged_species($spc, "HCHO");
                $data{$mechanism}{$speciation}{$run} = get_data($mecca, $kpp, $tagged_species, $mechanism, $speciation, $run, $no_runs);
            }
        }
    }
}
...
foreach my $mechanism (sort keys %data) {
    ...
    foreach my $speciation (sort keys %{$data{$mechanism}}) {
        ...
        foreach my $process (sort keys %{$data{$mechanism}{$speciation}}) {
            ...
            $R->set('HCHO.production', $data{$mechanism}{$speciation}{$process});
...
$R->run(q` CairoPDF(file = "Cumulative_HCHO_budget_allocated_facet_mechanism.pdf", width = 8.6, height = 6) `,
        q` print(plot) `,
        q` dev.off() `,
);
...
$R->run(q` CairoPDF(file = "Cumulative_HCHO_budget_allocated_facet_speciation.pdf", width = 8.6, height = 6) `,
        q` print(plot) `,
        q` dev.off() `,
);
...
sub get_data {
    my ($mecca, $kpp, $species, $mechanism, $speciation, $run, $no_runs) = @_;
    my $ntime = $mecca->time->nelem;
    my (%production_rates, %consumption_rates);
    $families{"HCHO_${mechanism}_$speciation"} = [ @$species ];
    $kpp->family({
            name    => "HCHO_${mechanism}_$speciation",
            members => $families{"HCHO_${mechanism}_$speciation"},
            weights => $weights{"HCHO_${mechanism}_$speciation"},
    });
    my $producers = $kpp->producing("HCHO_${mechanism}_$speciation");
    my $producer_yields = $kpp->effect_on("HCHO_${mechanism}_$speciation", $producers);
    my $consumers = $kpp->consuming("HCHO_${mechanism}_$speciation");
    my $consumer_yields = $kpp->effect_on("HCHO_${mechanism}_$speciation", $consumers);
    print "No consumers for HCHO in $mechanism and $speciation\n" if (@$consumers == 0);
    print "No producers for HCHO in $mechanism and $speciation\n" if (@$producers == 0);
    ...
    foreach my $process (keys %production_rates) {
        ...
        my $sum = $production_rates{$process}->sum;
