#! /usr/bin/env perl
# Daily Higher alkanes HO2x production budgets
...
$R->run(q` CairoPDF(file = "Higher_alkanes_HO2x_production_budgets_facet_speciation.pdf", width = 10, height = 7) `,
        q` print(plot) `,
        q` dev.off() `,
);
...
sub get_data {
    ...
    for (0..$#$producers) {
        ...
        next unless (defined $parent and $parent ~~ @higher_alkanes);
